...
const Notes = ({ data,handleXCLick }) => {   
    ...
    const handleDescriptionText = (des,limit=5) =>{
        const word = des.split(" ");
        return word.length > limit ? word.slice(0,limit).join(" ") + "..." : des;
    }

    return (
        <div className="md:w-1/2 sm:w-full  h-full pt-5 pl-5 flex flex-wrap gap-5 overflow-auto">
            <h4 className='w-full'>Recent Notes</h4>
            {
               data.length >= 1 ?  data.map((item, index) => (
                    <div key={index} className='relative w-40 h-50 bg-white cursor-pointer'>
                        <div className='flex justify-between px-3 pt-2'>
                            ...
                            {
                                item.Important ? <small className='bg-amber-400 text-white py-1 px-1 rounded upprcase'>IMP</small> : <small className='mt-6'></small>
                            }
                        </div>
                        <div className='my-3 mx-2'>
                            <h2 className='font-bold text-lg mb-1'>{item.Title}</h2>
                            <p className='text-gray-500'>{handleDescriptionText(item.Desc,5)}</p>
                        </div>
                    </div>
